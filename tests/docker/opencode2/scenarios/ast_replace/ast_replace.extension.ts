...
const expectedIds = ["ast_replace/T1/happy","ast_replace/T2/invalid_arguments","ast_replace/T2/missing_target","ast_replace/T7/happy"];
async function validate(context: HarnessValidationContext): Promise<void> {
  const scenarios = context.scenarios.filter((scenario) => scenario.tool === "ast_replace");
  const actual = scenarios.map((scenario) => scenario.id).sort();
  if (JSON.stringify(actual) !== JSON.stringify(expectedIds)) throw new Error("ast_replace" + " scenario identity mismatch: " + actual.join(","));
  ...
  if (JSON.stringify(matrix.rows?.[0]?.trajectories) !== JSON.stringify({"T1":"expected_fail:https://github.com/anomalyco/opencode/issues/37164","T2":"applicable","T3":"n/a:no-permission-operation","T4":"n/a:no-abortable-operation","T5":"n/a:no-background-capability","T6":"n/a:no-list-surface","T7":"expected_fail:https://github.com/anomalyco/opencode/issues/37164"})) throw new Error("ast_replace" + " applicability mismatch");
  const controls = JSON.parse(await readFile(join(here, "mutation-controls.json"), "utf8"));
  if (!Array.isArray(controls.controls) || controls.controls.length < 3) throw new Error("ast_replace" + " mutation controls missing");
}
...
const extension: HarnessExtension = { name: "ast_replace-scenarios-v1", validate, beforeScenario };
