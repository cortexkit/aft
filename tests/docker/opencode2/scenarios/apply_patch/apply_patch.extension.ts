...
const expectedIds = ["apply_patch/T1/happy","apply_patch/T2/invalid_arguments","apply_patch/T2/missing_target","apply_patch/T3/apply_patch_ask_allow","apply_patch/T3/apply_patch_ask_deny","apply_patch/T3/apply_patch_config_deny","apply_patch/T7/happy"];
async function validate(context: HarnessValidationContext): Promise<void> {
  ...
  if (JSON.stringify(matrix.rows?.[0]?.trajectories) !== JSON.stringify({"T1":"expected_fail:https://github.com/anomalyco/opencode/issues/37164","T2":"applicable","T3":"expected_fail:https://github.com/anomalyco/opencode/issues/37164","T4":"n/a:no-abortable-operation","T5":"n/a:no-background-capability","T6":"n/a:no-list-surface","T7":"expected_fail:https://github.com/anomalyco/opencode/issues/48340"})) throw new Error("apply_patch" + " applicability mismatch");
  const controls = JSON.parse(await readFile(join(here, "mutation-controls.json"), "utf8"));
  if (!Array.isArray(controls.controls) || controls.controls.length < 3) throw new Error("apply_patch" + " mutation controls missing");
}
...
const extension: HarnessExtension = { name: "apply_patch-scenarios-v1", validate, beforeScenario };
