...
const expectedIds = ["edit/T1/happy","edit/T2/invalid_arguments","edit/T2/missing_target","edit/T3/edit_ask_allow","edit/T3/edit_ask_deny","edit/T3/edit_config_deny","edit/T7/happy"];
async function validate(context: HarnessValidationContext): Promise<void> {
  const scenarios = context.scenarios.filter((scenario) => scenario.tool === "edit");
  const actual = scenarios.map((scenario) => scenario.id).sort();
  if (JSON.stringify(actual) !== JSON.stringify(expectedIds)) throw new Error("edit" + " scenario identity mismatch: " + actual.join(","));
  ...
  if (JSON.stringify(matrix.rows?.[0]?.trajectories) !== JSON.stringify({"T1":"expected_fail:https://github.com/anomalyco/opencode/issues/37164","T2":"applicable","T3":"expected_fail:https://github.com/anomalyco/opencode/issues/37164","T4":"n/a:no-abortable-operation","T5":"n/a:no-background-capability","T6":"n/a:no-list-surface","T7":"expected_fail:https://github.com/anomalyco/opencode/issues/37164"})) throw new Error("edit" + " applicability mismatch");
  const controls = JSON.parse(await readFile(join(here, "mutation-controls.json"), "utf8"));
  if (!Array.isArray(controls.controls) || controls.controls.length < 3) throw new Error("edit" + " mutation controls missing");
}
...
const extension: HarnessExtension = { name: "edit-scenarios-v1", validate, beforeScenario };
