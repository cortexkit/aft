...
async function validate(context: HarnessValidationContext): Promise<void> {
  const scenarios = context.scenarios.filter((scenario) => scenario.tool === "inspect");
  const actual = scenarios.map((scenario) => scenario.id).sort();
  if (JSON.stringify(actual) !== JSON.stringify(expectedIds)) throw new Error("inspect" + " scenario identity mismatch: " + actual.join(","));
  ...
  if (JSON.stringify(matrix.rows?.[0]?.trajectories) !== JSON.stringify({"T1":"applicable","T2":"applicable","T3":"n/a:no-permission-operation","T4":"applicable","T5":"n/a:no-background-capability","T6":"applicable","T7":"applicable"})) throw new Error("inspect" + " applicability mismatch");
