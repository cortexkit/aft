...
const expectedIds = ["grep/T1/happy","grep/T2/invalid_arguments","grep/T2/missing_target","grep/T6/grep-payload-matches/complete","grep/T6/grep-payload-matches/incomplete","grep/T7/happy"];
async function validate(context: HarnessValidationContext): Promise<void> {
  const scenarios = context.scenarios.filter((scenario) => scenario.tool === "grep");
  const actual = scenarios.map((scenario) => scenario.id).sort();
  if (JSON.stringify(actual) !== JSON.stringify(expectedIds)) throw new Error("grep" + " scenario identity mismatch: " + actual.join(","));
  ...
  if (JSON.stringify(matrix.rows?.[0]?.trajectories) !== JSON.stringify({"T1":"expected_fail:https://github.com/anomalyco/opencode/issues/37164","T2":"applicable","T3":"n/a:no-permission-operation","T4":"n/a:no-abortable-operation","T5":"n/a:no-background-capability","T6":"expected_fail:https://github.com/anomalyco/opencode/issues/37164","T7":"expected_fail:https://github.com/anomalyco/opencode/issues/48340"})) throw new Error("grep" + " applicability mismatch");
  const controls = JSON.parse(await readFile(join(here, "mutation-controls.json"), "utf8"));
  if (!Array.isArray(controls.controls) || controls.controls.length < 3) throw new Error("grep" + " mutation controls missing");
}
...
const extension: HarnessExtension = { name: "grep-scenarios-v1", validate, beforeScenario };
