...
| LSP failure reasons that embed a server stderr tail (`lsp/manager.rs:2191`, `:2927`) | ERROR (`:2927` path only) | Third-party server stderr. Checked in S2 [src]: the initialize-failure reason (`:2927`) comes back from `spawn_server` as `LspError::ServerNotReady` and is logged once per (server, root) by `slog_error!("failed to spawn {}: {}")` in `ensure_server_for_file_detailed`; `failed_spawns` suppresses repeats. The post-initialize-exit reason (`:2191`) and the applicable-server path only reach tool responses (`lsp_diagnostics`/`lsp_inspect` server status), not the log. The tail is bounded twice: 64 lines of at most 4 KB in `lsp/client.rs`, then `STDERR_REASON_BYTES` (2 KB) in the reason, plus a fixed hint. | No extra truncation needed. The line passes through the module redactor like every durable line (S2 applies it in the sink). |
