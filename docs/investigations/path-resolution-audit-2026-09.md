...
| `gh` shim state | `(AFT_GH_SHIM_STATE_DIR, non-empty and absolute)`; otherwise `storage_dir(None)/gh-shim` | Fixed: no independent `XDG_STATE_HOME` universe. Dedicated state remains available for embedding/tests. |
