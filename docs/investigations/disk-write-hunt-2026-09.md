...
After (same copied input and refresh procedure, base + delta writer, measured with the release-profile test binary):

| Changed files | Elapsed | Artifact bytes read | Physical bytes / MiB | Logical bytes / MiB | Resulting base + log bytes / MiB |
| ---: | ---: | ---: | ---: | ---: | ---: |
| 1 | **15 ms** | **260** | **16,384 / 0.016** | 147,456 / 0.141 | 154,987,951 / 147.808 |
| 10 | **25 ms** | **260** | **57,344 / 0.055** | 180,224 / 0.172 | 155,028,325 / 147.847 |
| 100 | **33 ms** | **260** | **462,848 / 0.441** | 589,824 / 0.562 | 155,433,968 / 148.233 |
| Forced compaction after the 100-file row | 1,037 ms | 155,433,968 | 145,817,600 / 139.063 | 146,882,388 / 140.078 | 145,813,162 / 139.058 |

`semantic.bin` now starts with the unchanged V6/V7 base snapshot and appends one segment per persisted refresh. A segment contains ordered file-id tombstones followed by the complete replacement metadata, chunks, and vectors for those file ids. Each frame is `magic + u64 payload length + BLAKE3 checksum + payload`. Readers apply checksum-valid frames in sequence. A partial or checksum-invalid final frame is treated as a torn tail, leaving the preceding base + frames loadable; an owning reader or the next writer truncates it to the last valid boundary, while borrowed/read-only openers neither mutate the file nor expose the torn refresh—they return the previous committed state. This length-prefix/checksum design avoids rewriting the 145–155 MiB base merely to publish a small refresh.

Refresh and removal paths accumulate their complete replacement path set in memory and clear it only after successful persistence. The append path validates only the fixed base header/fingerprint and walks frame length prefixes; it verifies the final frame checksum because only that frame can be torn. The measured artifact had no prior segments in each independent row, so each append read only its 260-byte header/fingerprint instead of decoding 154,982,730 bytes. Structural diff remains a recovery fallback when mutation provenance is unavailable (a manually constructed index), or when the remembered artifact identity was replaced externally, including the first append after another process compacts the artifact.
...
Append and compaction share an inter-process persistence lock. Append wait time is at least five seconds and grows by one second per 32 MiB of artifact plus a two-second margin (seven seconds for this artifact), so a normal compaction does not cause a refresh delta to be abandoned; dirty paths remain pending after any failed attempt. The compactor logs scheduling, start, and finish with root, segment count, segment bytes, output bytes, and elapsed time. It reloads the committed base + segments after taking the lock and compares the scheduled file identity before replacing anything, so a stale background job cannot erase a later append. It writes and fsyncs `semantic.bin.tmp.*`, then atomically renames it over `semantic.bin` and syncs the parent directory. A reader that opened before the rename remains on the complete old inode; one that opens afterward receives the complete compacted inode. The same `read_from_disk_borrow_tolerant` path used by `readonly_artifacts.rs` hashes and loads the base plus segments, and never truncates a torn tail.

Structural regression coverage compares canonical serialized state (file table, chunks, vector bits, and fingerprint) with a fresh whole rebuild after cold build, 1-file refresh, 10-file refresh, three deletions, 100-file refresh, and compaction. At each refresh it also proves the dirty-path frame is byte-equal to the structural-diff frame. Separate guards cover tombstone masking, ordered supersession, one-copy compaction, the 64-segment bound, SIGKILL during a partial frame, a second process opening while compaction is paused immediately before the atomic swap, and an append waiting behind that paused compaction before landing on its replacement base.
