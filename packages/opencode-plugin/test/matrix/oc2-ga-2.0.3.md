...
Compared with beta-19234, the permission surface did not gain a request endpoint. The previously applicable T6 rows for `bash`, `glob`, `grep`, and `search` are now recorded as #37164 expected failures because their incomplete fixture cannot execute far enough to emit its trailer. GA also has no `/api/session/:sessionID/abort` route in the published CLI route table (the table proceeds from `/api/session/:sessionID/wait` through revert/context routes); `inspect/T4` records that host-404 as an expected failure against the exact 2.0.3 upstream artifact.
