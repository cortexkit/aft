...
interface GithubInstalledMeta {
  version: string;
  installedAt: string;
  /** Steady-state hash of the final binary under bin/. */
  sha256?: string;
  binarySha256?: string;
 ...
 * forces us to fail closed if extracted bytes ever exceed a reasonable
 * upper bound for an LSP server bundle.
 *
 * Real LSP archives extract to far less than 1 GB — clangd ~150 MB,
 * lua-language-server ~80 MB, zls ~30 MB. 1 GB leaves headroom for
 * future bundles while making decompression bombs detectable.
 */
...
async function resolveTargetTag(
  ...
  //       (60/hr rate limit), risking probe failures on rapid session
  //       reopens.
  //   (b) When the verification probe failed, code fell through to the
  //       LIVE probe below, and on its failure returned
  //       `{ tag: cached.latest_eligible, assets: [] }` — a non-null tag
  //       with empty assets. Callers proceed to downloadAndInstall, hit
  //       `assets.find(...) === undefined`, and emit misleading
  //       "asset not found" warnings when the real issue is network.
  //
  // Fix: when cache is fresh, trust it and skip the network probe entirely.
  // The next refresh happens after grace_days. This matches the npm path
  // (resolveTargetVersion) which already short-circuits on fresh cache.
  ...
  if (cached && !shouldRecheckVersion(cached, weeklyMs) && cachedSafe) {
    ...
    // Cached tag now unreachable — fall through to live probe.
  }
  ...
  if (!probe) {
    ...
    // the caller skip cleanly with a "github releases probe failed" reason.
    return {
      tag: null,
      assets: [],
      blockedByGrace: false,
...
function validateCachedGithubInstall(spec: GithubServerSpec, platform: Platform): boolean {
  ...
  // Lazy migration for installs created before this fix: older metadata stored
  // the archive hash in `sha256`, but steady-state validation hashes the final
  // extracted binary. Preserve the old value as archiveSha256 and start using
  // the binary hash for future cache validation.
