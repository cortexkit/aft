...
- Location: `crates/aft/src/list_envelope.rs`, `crates/aft/src/list_surfaces.rs`, `crates/aft/src/ndjson_text.rs`
- Pattern: Registry-backed truncation envelope defining reason precedence (Walk > Depth > Budget > Cap), exact or lower-bound totals (`Total::Exact`, `Total::AtLeast`), standardized unit words, narrow parameter hints, and pinned trailer grammar (`shown <n> of <total> <unit> (<reason>) · narrow: <a>, <b>`). Emits trailer lines iff truncated and clamps displayed totals at or above `1e9` to `≥999999999`.
