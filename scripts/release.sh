...
if mkdir -p "$CACHE_DIR/$TAG" && cp target/release/aft "$CACHE_DIR/$TAG/aft" 2>/dev/null; then
  ...
  if [[ "$(uname -s)" == "Darwin" ]]; then
    ...
    # Never write to target/release/aft.dSYM: cargo owns that name as a
    # symlink into deps/ and unlinks it on the next build, which fails with
    # EPERM once a real bundle sits there and breaks every later release build
    # in the checkout. The generated bundle takes a name cargo never touches.
    RELEASE_DSYM="target/release/ck-aft-release.dSYM"
    rm -rf "$RELEASE_DSYM"
    dsymutil target/release/aft -o "$RELEASE_DSYM"
    if [[ -d "$RELEASE_DSYM" ]]; then
      IMAGE_UUID="$(dwarfdump --uuid "$CACHE_DIR/$TAG/aft" | awk 'NR == 1 { gsub(/-/, "", $2); print toupper($2) }')"
      DSYM_UUID="$(dwarfdump --uuid "$RELEASE_DSYM" | awk 'NR == 1 { gsub(/-/, "", $2); print toupper($2) }')"
      ...
      ditto "$RELEASE_DSYM" "$DSYM_TMP/aft.dSYM"
      rm -rf "$DSYM_DEST"
      ...
      mkdir -p "$PRESWAP_DIR"
      ditto "$RELEASE_DSYM" "$PRESWAP_DIR/aft-${BIN_SHA}.dSYM"
