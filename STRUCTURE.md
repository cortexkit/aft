...
**`crates/aft/src/list_surfaces/`:**
- Purpose: Register list-shaped surfaces, define truncation metadata, and construct list envelopes across commands.
- Contains: Surface adapters for callgraph, trace, inspect, bash, grep, glob, search, and outline.
- Key files: `crates/aft/src/list_surfaces.rs`, `crates/aft/src/list_surfaces/callgraph.rs`, `crates/aft/src/list_surfaces/inspect.rs`, `crates/aft/src/list_surfaces/bash.rs`, `crates/aft/src/list_surfaces/trace.rs`
...
**New list-shaped surface or truncation adapter:** `crates/aft/src/list_surfaces/[surface].rs` -- register the surface in `crates/aft/src/list_surfaces.rs` and implement its envelope constructor.

**New Windows path normalization rule:** `crates/aft/src/windows_path.rs` -- add non-verbatim conversion or path normalization logic for Windows extended-length namespaces.
