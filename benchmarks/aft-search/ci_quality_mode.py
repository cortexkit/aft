#!/usr/bin/env python3
"""Select the narrow manifest-only verification exception from one full diff."""
...
def select(paths:list[str])->str:
    ...
    allowed=lambda path: path==manifest
...
def main()->int:
    ...
    if args.self_test:
        ...
        assert select(["benchmarks/aft-search/real-query-manifest.json","benchmarks/aft-search/bundles/a.zip"])=="evaluate"
        assert select(["benchmarks/aft-search/real-query-manifest.json","benchmarks/aft-search/provision_evidence.py"])=="evaluate"
