...
fn rust_crate_roots_resolve_bin_and_other_target_module_trees() {
    ...
    let exact_tool_sites = BTreeSet::from([
        ("src/bin/support/admin_client.rs".to_string(), 4_i64, "route_client::error_reason".to_string()),
        ("src/bin/support/admin_client.rs".to_string(), 8, "route_client::error_reason".to_string()),
        ("src/bin/support/admin_client.rs".to_string(), 12, "super::route_client::error_reason".to_string()),
        ("src/bin/support/admin_client.rs".to_string(), 16, "crate::support::route_client::error_reason".to_string()),
        ("src/bin/support/route_client.rs".to_string(), 6, "error_reason".to_string()),
    ]);
    assert_eq!(
    ...
    assert!(entries.iter().all(|entry| {
        entry["approximate"] == false && entry["resolved_by"] == "treesitter+resolver"
    ...
    assert_target_site(&store, "src/bin/other/nested.rs", "nested_target", "src/bin/other/main.rs", 4, "nested::nested_target");
    assert_target_site(&store, "tools/custom_support.rs", "custom_target", "tools/custom.rs", 4, "custom_support::custom_target");
    assert_target_site(&store, "examples/demo_support.rs", "example_target", "examples/demo.rs", 4, "demo_support::example_target");
    assert_target_site(&store, "tests/test_support.rs", "integration_target", "tests/standalone.rs", 5, "test_support::integration_target");
    assert_target_site(&store, "benches/bench_support.rs", "bench_target", "benches/perf.rs", 4, "bench_support::bench_target");
    ...
    .unwrap();
    assert_eq!(lib_callers["total_callers"], 1, "lib cfg(test) caller must stay in the lib crate tree: {lib_callers:#}");
    assert_target_site(&store, "src/lib.rs", "lib_target", "src/lib.rs", 9, "super::lib_target");
    ...
    let view_sites = joined
        .rows
        .iter()
        .filter(|row| row.target_path.as_deref() == Some(b"src/bin/support/route_client.rs") && row.target_symbol.as_deref() == Some("error_reason"))
        .map(|row| {
            let blob = CallgraphBlob::from_bytes(&blobs.0[&row.caller_blob_key]).unwrap();
            let raw = blob.parse().unwrap().refs.iter().find(|raw| raw.ordinal == row.ref_ordinal).unwrap();
            (
    ...
    let expected_view_sites = exact_tool_sites
        ...
        .collect();
    assert_eq!(view_sites, expected_view_sites, "ProjectFacts resolver must match disk resolution");
}

fn assert_target_site(store: &CallGraphStore, target_file: &str, target_symbol: &str, caller_file: &str, line: i64, call_text: &str) {
    assert_eq!(
...
fn resolved_sites(store: &CallGraphStore, target_file: &str, target_symbol: &str) -> BTreeSet<(String, i64, String)> {
    let conn = rusqlite::Connection::open(store.sqlite_path()).unwrap();
    let mut stmt = conn.prepare(
        "SELECT r.caller_file, r.line, r.full_ref FROM refs r JOIN edges e ON e.ref_id = r.ref_id WHERE e.target_file = ?1 AND e.target_symbol = ?2 AND r.status = 'resolved' AND e.provenance = 'treesitter+resolver' ORDER BY r.caller_file, r.line",
    ).unwrap();
    stmt.query_map([target_file, target_symbol], |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?)))
        .unwrap()
        .map(Result::unwrap)
        .collect()
}
    ...
    for path in paths {
        ...
        entries.push((
            ...
            ManifestEntry::Regular {
                mode: 0o100644,
                planes: RegularPlanes { semantic: None, callgraph: Some(key) },
                resolution_input: path.file_name().and_then(|name| name.to_str()) == Some("Cargo.toml"),
            },
        ));
    }
