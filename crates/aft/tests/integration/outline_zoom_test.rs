...
"#,
    ...
    // The call lists below come from the file's own syntax tree and are
    // returned even though this process has no callgraph index; the explicit
    // callgraph field reports that index's status.
    assert_eq!(resp["callgraph"]["status"], "unavailable");
    assert_eq!(resp["callgraph"]["code"], "callgraph_unavailable");
