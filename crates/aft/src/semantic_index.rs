...
/// unexplained text. packages/aft-bridge/src/semantic-status.ts checks its own
/// mapping against this list, so adding a word here without teaching the
/// readers about it fails that test instead of shipping.
///
/// `empty` is absent on purpose: it was the loaded index object's entry count
/// wearing a status word, and it is no longer emitted. Readers still recognise
/// it, because a daemon older than that change can still send it.
...
impl SemanticIndex {
    ...
    ///
    /// The daemon-held status decides the word, and nothing else does.
    /// Deciding it from the index's entry count could not say "failed" at all,
    /// so `aft status` answered "is semantic search working?" with `ready` for
    /// a root whose index had died — while the sidebar and a search reply both
    /// reported the failure correctly. The surface a user checks first was the
    /// one surface that could not say no.
    ///
    /// An entry count of zero is not a status: a ready index with nothing in it
    /// is still queryable and still answers `ready`, which is what callers
    /// waiting for readiness have always polled for. A build in progress
    /// reports `loading` even when a previous index object is still installed,
    /// because that object is not what the daemon is serving from.
...
"#,
    ...
    /// Readers check their rendering against this list, so the list has to
    ...
    fn semantic_status_words_include_the_labels_this_module_produces() {
        ...
        assert!(
            !SEMANTIC_INDEX_STATUS_WORDS.contains(&"empty"),
            "nothing emits `empty` any more; listing it asks readers to cover a word that never arrives"
        );
    ...
    /// working?". While this label was decided by the index's entry count it
    /// had no way to say no: a root whose index had failed reported `ready`
    /// there (or `empty`, for a failure that left nothing behind) while the
    /// sidebar and a search reply both reported the failure.
    #[test]
    fn failed_index_reports_the_failure_instead_of_its_entry_count() {
        ...
        // A ready index answers `ready` whether or not it holds anything: it is
        // queryable either way, and callers poll this word for readiness.
        for index in [&populated, &empty] {
            assert_eq!(index.status_label(&SemanticIndexStatus::ready()), "ready");
        }
        assert_eq!(
